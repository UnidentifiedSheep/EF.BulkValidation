        ...
        (ImmutableArray<IPropertySymbol?> left, string right) tuple)
    {
        ...
        foreach (var propertySymbol in properties)
        {
            ...
            foreach (var rule in _baseRules)
            {
                ...
                var functionName = $"Validate{shortClassName}{rule.shortcut}{shortPropertyName}";
                ...
                sb.AppendLine($$"""
                                        ...
                                        this IValidationPlan plan, {{keyTypeStr}} value {{additionalArgs}})
                                    {
                                        ...
                                        plan.Add(new global::{{rule.@namespace}}<{{longClassName}}>(keyValue), "{{functionName}}");
                                        return plan;
                                    }
                                """);
                sb.AppendLine($$"""
                                        ...
                                        this IValidationPlan plan{{additionalArgs}}, params {{keyTypeStr}}[] values)
                                    {
                                        ...
                                        plan.Add(new global::{{rule.@namespace}}<{{longClassName}}>(keyValues), "{{functionName}}");
                                        return plan;
                                    }
                                """);
                sb.AppendLine($$"""
        ...
        context.AddSource("ValidationFunctions.g.cs", $$"""
                                                        using BulkValidation.Core.Models;
                                                        namespace {{rootNamespace}}
                                                        {
                                                            public sealed partial class ValidationFunctions
                                                            {
                                                                {{string.Join(";\n", functionNames.Select(x => $"public const string {x} = \"{x}\""))}};
                                                            }
                                                        }
                                                        """);
    }
